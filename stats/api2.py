...
# TODO: Strip quotations from event venues
        ...
        if event['event_id'] not in existing_event_ids:
            ...
            for _, row in data.iterrows():
                ...
                if row['prog_name'] in male_progs and row['results'] and row['is_race']:
                    ...
                    prog_details["race_venue"] = event["event_venue"]
                    prog_details["race_country"] = event["event_country"]
...
def get_new_female_short_progs():
    # Get all events
    events = pd.read_csv(os.path.join("data", "events.csv"), header = 0)
